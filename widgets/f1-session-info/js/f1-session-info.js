...
vis.binds["f1"].createSessionWidget = function (el, view, data, style) {
    ...
    function applySession(sessionData) {
        ...
        $type.append(
            ...
            '<span>' + (sessionData.session_type || 'Session') + '</span>' +
            '</div>'
        );
        ...
        if (sessionData.track_status) {
            ...
            var flagClass = trackStatus === "allclear" ? "green" : 
                            (trackStatus.includes("sc") || trackStatus === "yellow") ? "yellow" :
                            trackStatus === "red" ? "red" : "";
            
            var $track = $('<div class="f1-session-section"></div>');
            $track.append(
                '<div class="f1-session-track-status">' +
                '<span class="f1-session-flag">' + flagIcon + '</span>' +
                '<span class="f1-session-flag-text ' + flagClass + '">' +
                sessionData.track_status.replace(/([A-Z])/g, ' $1').trim().toUpperCase() +
                '</span>' +
                '</div>'
            );
        ...
        if (sessionData.weather) {
            ...
            if (w.air_temperature != null) {
                $grid.append(
                    ...
                    '<div class="f1-session-weather-value">' + w.air_temperature + '°C</div>' +
                    '</div>'
                );
            }
            if (w.track_temperature != null) {
                $grid.append(
                    ...
                    '<div class="f1-session-weather-value">' + w.track_temperature + '°C</div>' +
                    '</div>'
                );
            }
            if (w.wind_speed != null) {
                ...
                $grid.append(
                    ...
                    '<div class="f1-session-weather-value">' + windText + '</div>' +
                    '</div>'
                );
            }
            if (w.humidity != null) {
                $grid.append(
                    ...
                    '<div class="f1-session-weather-value">' + w.humidity + '%</div>' +
                    '</div>'
                );
            }
    ...
    function applyRaceControl(message) {
        ...
        if (message) {
            var $msg = $(
                ...
                '<span class="f1-session-message-text">' + message + '</span>' +
                '</div>' +
                '</div>'
            );
            $content.append($msg);
    ...
    function loadSession() {
        vis.conn.getStates(oidSession, function (err, states) {
            if (!err && states && states[oidSession] && states[oidSession].val) {
                try {
                    var session = typeof states[oidSession].val === "string"

	...
	function applyLaps(laps) {
		...
		sorted.slice(0, 10).forEach(function (lap) {
			...
			if (lap.duration_sector_1 || lap.duration_sector_2 || lap.duration_sector_3) {
				...
				$sectors.append(
					'<span class="f1-laptimes-sector">S1: ' + formatTime(lap.duration_sector_1) + "</span>",
				);
				$sectors.append(
					'<span class="f1-laptimes-sector">S2: ' + formatTime(lap.duration_sector_2) + "</span>",
				);
				$sectors.append(
					'<span class="f1-laptimes-sector">S3: ' + formatTime(lap.duration_sector_3) + "</span>",
				);
				$row.append($sectors);
			}
	...
	function loadLaps() {
		vis.conn.getStates(oidLaps, function (err, states) {
			if (!err && states && states[oidLaps] && states[oidLaps].val) {
				try {
					var laps =
						typeof states[oidLaps].val === "string" ? JSON.parse(states[oidLaps].val) : states[oidLaps].val;
					applyLaps(laps);
				} catch (e) {}
			}
		});

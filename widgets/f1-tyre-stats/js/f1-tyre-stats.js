	...
	function applyData() {
		...
		drivers.forEach(function (driver) {
			...
			if (current.compound) {
				...
				$current.append(
					'<div class="f1-tyre-compound ' +
						current.compound +
						'" style="' +
						(current.compound === "SOFT"
							? "background:#ff0000;color:#fff;"
							: current.compound === "MEDIUM"
								? "background:#ffff00;color:#1a1a1a;"
								: current.compound === "HARD"
									? "background:#ffffff;color:#1a1a1a;"
									: current.compound === "INTERMEDIATE"
										? "background:#00ff00;color:#1a1a1a;"
										: current.compound === "WET"
	...
	function loadStints() {
		vis.conn.getStates(oidStints, function (err, states) {
			if (!err && states && states[oidStints] && states[oidStints].val) {
				try {
					var stints =
						typeof states[oidStints].val === "string"
	...
	function loadCurrent() {
		vis.conn.getStates(oidCurrent, function (err, states) {
			if (!err && states && states[oidCurrent] && states[oidCurrent].val) {
				try {
					var current =
						typeof states[oidCurrent].val === "string"

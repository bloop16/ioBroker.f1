...
vis.binds["f1"].createRaceControlWidget = function (el, view, data, style) {
	...
	function detectFlag(message) {
		...
		if (lower.includes("yellow flag") || lower.includes("safety car") || lower.includes("vsc"))
			return "yellow-flag";
	...
	function applyMessages(messagesData) {
		...
		sorted.forEach(function (msg) {
			...
			var $msg = $(
				'<div class="f1-race-control-message ' +
					flagClass +
					'">' +
					'<span class="f1-race-control-message-flag">' +
					flagEmoji +
					"</span>" +
					'<div class="f1-race-control-message-content">' +
					'<div class="f1-race-control-message-text">' +
					(msg.message || "") +
					"</div>" +
					'<div class="f1-race-control-message-time">' +
					timeStr +
					"</div>" +
					"</div>" +
					"</div>",
	...
	function loadMessages() {
		vis.conn.getStates(oidMessages, function (err, states) {
			if (!err && states && states[oidMessages] && states[oidMessages].val) {
				try {
					var messages =
						typeof states[oidMessages].val === "string"

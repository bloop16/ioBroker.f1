...
const jolpica_client_js_1 = require("./lib/jolpica-client.js");
const session_tracker_js_1 = require("./lib/session-tracker.js");
const signalr_client_js_1 = require("./lib/signalr-client.js");
const state_mapper_js_1 = require("./lib/state-mapper.js");
...
class F1 extends utils.Adapter {
    jolpika = new jolpica_client_js_1.JolpikaClient();
    tracker = new session_tracker_js_1.SessionTracker();
    mapper = new state_mapper_js_1.StateMapper();
    signalR;
    scheduleTimer;
    sessionCheckTimer;
    ...
    constructor(options = {}) {
        ...
        this.signalR = new signalr_client_js_1.SignalRClient(this.log);
    ...
    async onReady() {
        ...
        this.signalR.on("message", (topic, data, timestamp) => {
            this.mapper.handle(this, topic, data, timestamp).catch((err) => {
        ...
        this.scheduleTimer = setInterval(() => {
            this.refreshScheduleAndStandings().catch((err) => {
        ...
        this.sessionCheckTimer = setInterval(() => {
            this.checkAndManageSignalR().catch((err) => {
    ...
    // ----------------------------------------------------------------
    async refreshScheduleAndStandings() {
        ...
        try {
            ...
            this.log.warn(`Failed to fetch schedule: ${err.message}`);
        }
        try {
            ...
            this.log.warn(`Failed to fetch standings: ${err.message}`);
        }
        ...
        if (!this.signalR.isConnected()) {
            try {
                ...
                this.log.debug(`Failed to fetch last results: ${err.message}`);
            }
        }
    ...
    async updateScheduleStates(races) {
        // Next race
        ...
        if (weekend) {
            ...
            await this.setStateAsync("weekend_sessions.sessions_json", {
                val: JSON.stringify(weekend.sessions.map(s => ({
        ...
        const now = new Date();
        const sorted = [...meetings].sort((a, b) => new Date(a.date_start).getTime() - new Date(b.date_start).getTime());
        const current = sorted.find(m => new Date(m.date_start) <= now && new Date(m.date_end) >= now) ??

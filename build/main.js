...
class F1 extends utils.Adapter {
    ...
    async initializeStates() {
        ...
        await this.setObjectNotExistsAsync("weekend_sessions", {
            type: "channel",
            common: { name: "Weekend Sessions Information" },
            native: {},
        });
        const weekendSessionsStates = [
            { id: "circuit", name: "Circuit Name", type: "string", role: "text" },
            { id: "country", name: "Country", type: "string", role: "text" },
            { id: "location", name: "Location", type: "string", role: "text" },
            ...
            { id: "sessions_json", name: "All Sessions (JSON)", type: "string", role: "json" },
        ];
        for (const state of weekendSessionsStates) {
    ...
    async updateStandings() {
        try {
            ...
            if (response.data && response.data.length > 0) {
                ...
                const teams = Array.from(new Map(drivers.map(d => [d.team_name, { name: d.team_name, colour: d.team_colour }])).values());
            ...
            this.log.error(`Failed to update standings: ${error instanceof Error ? error.message : String(error)}`);
        }
    }

...
class F1 extends utils.Adapter {
	...
	private coreHandshakeAck = false;
	private coreSubscribeSent = false;
	...
	private topThreeData: Record<string, any> = {};
	...
	private clockRefUtcMs = 0;
	private clockRefRemainingMs = 0;
	private clockExtrapolating = false;
	private clockInterval?: ioBroker.Interval;
	...
	private async updateScheduleStates(
		...
		const toUTC = (date: string, time: string): Date => {
	...
	private async connectSignalR(): Promise<void> {
		...
		try {
			...
			const negRes = await this.ltApi.post<{ connectionToken?: string }>(SIGNALR_CORE_NEGOTIATE_URL, null, {
				params: { negotiateVersion: "1" },
			...
			this.ws = new WebSocket(`${SIGNALR_CORE_WS_URL}?id=${encodeURIComponent(rawToken)}`, {
				headers: wsHeaders,
			});
	...
	private sendCoreSubscribeOnce(): void {
		if (this.coreSubscribeSent || !this.ws || this.ws.readyState !== WebSocket.OPEN) {
			return;
		}
		this.ws.send(
			`${JSON.stringify({
	...
	private async handleWsMessage(raw: string): Promise<void> {
		...
		for (const chunk of chunks) {
			...
			if (payload?.type === 1 && payload?.target === "feed" && Array.isArray(payload?.arguments)) {
				const [stream, data] = payload.arguments as [string, any];
	...
	private startClockExtrapolation(): void {
	...
	private stopClockExtrapolation(): void {
		if (this.clockInterval) {
	...
	private parseRemainingToMs(str: string): number {
	...
	private async onTopThree(data: any): Promise<void> {
		...
		if (Array.isArray(data.Lines)) {
			...
			this.topThreeData = {};
			(data.Lines as any[]).forEach((l: any, i: number) => {
			...
			this.topThreeData = this.deepMerge(this.topThreeData, data.Lines as Record<string, any>);
		}
		const entries = Object.values(this.topThreeData);
		if (entries.length === 0) {
			return;
		}
		const top3 = entries.slice(0, 3).map((l: any) => ({
	...
	private async onTeamRadio(data: any): Promise<void> {
		const incoming: any[] = data?.Captures ? Object.values(data.Captures) : Array.isArray(data) ? data : [];
		if (incoming.length === 0) {
			return;
		}
	...
	private async onPitStops(data: any): Promise<void> {
		...
		const driverMap: Record<string, any> =
			data?.Stops && typeof data.Stops === "object" && !Array.isArray(data.Stops) ? data.Stops : data;
		...
		for (const [num, raw] of Object.entries(driverMap ?? {})) {
			...
			if (existing.length === 0) {
				...
				for (const p of incoming) {
					...
					const isDuplicate =
						lapKey != null && existing.some(e => (e.Lap ?? e.lap ?? e.LapNumber) === lapKey);
					if (!isDuplicate) {
	...
	private async onTyreStints(data: any): Promise<void> {
		...
		//   {"1": [{...}], "3": [{...}], ...}
		const driverMap: Record<string, any> =
			data?.Stints && typeof data.Stints === "object" && !Array.isArray(data.Stints) ? data.Stints : data;
